...
export default function App() {
  ...
  // Référentiel EDF B13-23 : Rc = (RM + RNi - RMN) / 2 puis c = Rc / RM.
  // Dans l'application, le champ Rc correspond à la "résistance de couplage" déjà calculée/affichée par le TERCA.
  // Le coefficient conforme au mémento est donc c = Rc / RM, et non Rc / RN.
  const kInitial = useMemo(() => {
    if (data.coeff) return n(data.coeff);
    return n(data.rm) ? n(data.rc) / n(data.rm) : 0;
  }, [data.coeff, data.rm, data.rc]);

  const kAfter1 = useMemo(() => n(data.rm2 || data.rm) ? n(data.rc2 || data.rc) / n(data.rm2 || data.rm) : 0, [data.rm2, data.rc2, data.rm, data.rc]);
  const kFinal = useMemo(() => n(data.rm3 || data.rm2 || data.rm) ? n(data.rc3 || data.rc2 || data.rc) / n(data.rm3 || data.rm2 || data.rm) : 0, [data.rm3, data.rc3, data.rm2, data.rc2, data.rm, data.rc]);
  ...
  const rnTarget = 50;
  const rcTarget = n(data.rm) * K_TARGET;
  const rcTargetAfter1 = n(data.rm2 || data.rm) * K_TARGET;
  ...
  const diagnostic = useMemo(() => {
    ...
    const causes = [];
    if (coeffMauvais) causes.push(`Coefficient K = ${kInitial.toFixed(3)} > 0,15 : Rc doit être inférieur à ${rcTarget.toFixed(2)} Ω maximum selon Rc inférieur à 0,15 × RM.`);
  ...
  const prises = useMemo(() => {
    ...
    if (diagnostic.coeffMauvais || diagnostic.proche || diagnostic.solConducteur) {
      list.push({
        ...
        target: `Objectif : RM ≤ ${rmTarget} Ω / Rc ≤ ${rcTarget.toFixed(2)} Ω / c inférieur à 0,15`,
        ...
        formula: 'c = Rc / RM : réduire Rc par distance + géométrie.'
      });
    }
    if (!diagnostic.solDifficile) {
      list.push({
        ...
        target: `Objectif : RM ≤ ${rmTarget} Ω puis contrôle c inférieur à 0,15`,
    ...
    if (n(data.rn) > rnTarget) {
      list.push({
        ...
        target: `Objectif : RN ≤ ${rnTarget} Ω avec contrôle c inférieur à 0,15`,
        detail: `Créer ou renforcer RN à distance de RM. Ne pas renforcer RN côté RM si le coefficient de couplage est mauvais. ${apportTerreVegetale}`,
        formula: 'Réq = 1 / (1/R1 + 1/R2 + ...), puis c = Rc / RM.'
      });
    }
    return list;
  ...
  const solutions = useMemo(() => {
    ...
    const target = `RM ≤ ${rmTarget} Ω • RN ≤ ${rnTarget} Ω • Rc ≤ ${rcTarget.toFixed(2)} Ω • c inférieur à 0,15`;
    ...
    const complementTarget = `Compléter jusqu’à RM ≤ ${rmTarget} Ω • RN ≤ ${rnTarget} Ω • Rc ≤ ${rcTargetAfter1.toFixed(2)} Ω • c inférieur à 0,15`;
    return [
  ...
  return <main className="min-h-screen bg-slate-950 p-4 text-slate-900"><div className="mx-auto max-w-7xl overflow-hidden rounded-3xl bg-white shadow-2xl"><header className="bg-gradient-to-r from-slate-950 via-blue-900 to-slate-800 p-6 text-white"><p className="text-xs font-bold uppercase tracking-[0.3em] text-blue-200">SECAB Couplage Expert V7 EDF B13-23</p><h1 className="mt-2 text-4xl font-black">Couplage Expert Terrain</h1><p className="mt-1 text-blue-100">Formules corrigées EDF B13-23 : Rc=(RM+RNi−RMN)/2 puis c=Rc/RM.</p><div className="mt-5 grid gap-3 md:grid-cols-4"><div className="rounded-2xl bg-white/10 p-4"><div className="text-xs">c initial</div><div className="text-3xl font-black">{kInitial.toFixed(3)}</div></div><div className="rounded-2xl bg-white/10 p-4"><div className="text-xs">Rc cible</div><div className="text-3xl font-black">{rcTarget.toFixed(2)} Ω</div></div><div className="rounded-2xl bg-white/10 p-4"><div className="text-xs">Rc à réduire</div><div className="text-3xl font-black">{rcToReduce.toFixed(2)} Ω</div></div><div className="rounded-2xl bg-white/10 p-4"><div className="text-xs">c après 1</div><div className="text-3xl font-black">{kAfter1.toFixed(3)}</div></div></div></header>{error && <div className="m-5 rounded-2xl border-l-4 border-red-600 bg-red-50 p-4 font-black text-red-700">⚠ {error}</div>}<div className="grid lg:grid-cols-[310px_1fr]"><aside className="space-y-2 bg-slate-100 p-5">{steps.map(([s,icon],i)=><button key={s} onClick={()=>setStep(i)} className={`w-full rounded-2xl p-3 text-left font-black ${step===i?'bg-blue-600 text-white':'bg-white'}`}>{icon} {i+1}. {s}</button>)}</aside><section className="p-6">
    {step===0 && <div><h2 className="text-3xl font-black">🏠 Accueil</h2><div className="grid gap-4 md:grid-cols-4"><div className="rounded-3xl bg-blue-50 p-5"><b>Objectif</b><p>c inférieur à 0,15.</p></div><div className="rounded-3xl bg-green-50 p-5"><b>Cuivre</b><p>25 mm².</p></div><div className="rounded-3xl bg-orange-50 p-5"><b>Photos</b><p>Obligatoires.</p></div><div className="rounded-3xl bg-purple-50 p-5"><b>Sol</b><p>Résistivité prise en compte.</p></div></div></div>}
    ...
    {step===2 && <div><h2 className="text-3xl font-black">📷 Mesures initiales</h2><div className="grid gap-4 md:grid-cols-3">{field('RM initiale (Ω)','rm')}{field('RN initiale (Ω)','rn')}{field('Rc TERCA initiale (Ω)','rc')}{field('Distance RM/RN (m)','distance')}{field('Résistivité ρ (Ω.m)','resistivite')}</div><div className="mt-4 rounded-2xl bg-blue-50 p-4 font-bold">c = Rc / RM = {kInitial.toFixed(3)} — Rc max = 0,15 × RM = {rcTarget.toFixed(2)} Ω</div><SolNotice/><div className="mt-4 grid gap-4 md:grid-cols-2">{photo('Mesure RM initiale','photoRm')}{photo('Mesure RN initiale','photoRn')}{photo('Mesure Rc TERCA initiale','photoRc')}{photo('Environnement / implantation','photoEnv')}</div></div>}
    ...
    {step===7 && <div><h2 className="text-3xl font-black">🔁 Recalcul</h2><div className="grid gap-4 md:grid-cols-3">{field('RM après amélioration 1','rm2')}{field('RN après amélioration 1','rn2')}{field('Rc après amélioration 1','rc2')}</div><div className="mt-4 rounded-2xl bg-blue-50 p-4 font-black">c après amélioration 1 : {kAfter1.toFixed(3)}</div><div className="mt-4 grid gap-4 md:grid-cols-3">{photo('RM après amélioration 1','photoRm2')}{photo('RN après amélioration 1','photoRn2')}{photo('Rc TERCA après amélioration 1','photoRc2')}</div><div className="mt-4 rounded-2xl bg-yellow-50 p-4 font-bold">Si complément nécessaire, la suite repart de : RM {after1Base.rm} Ω • RN {after1Base.rn} Ω • RC {after1Base.rc} Ω • K {after1Base.k.toFixed(3)}.<br/>Ordre : réduire RC/couplage → vérifier parasites → compléter RM → renforcer RN seulement si nécessaire.</div><div className="mt-4 grid gap-4 md:grid-cols-3">{field('RM après complément','rm3')}{field('RN après complément','rn3')}{field('Rc après complément','rc3')}</div><div className="mt-4 rounded-2xl bg-purple-50 p-4 font-black">c final : {kFinal.toFixed(3)}</div><div className="mt-4 grid gap-4 md:grid-cols-3">{photo('RM après complément','photoRm3')}{photo('RN après complément','photoRn3')}{photo('Rc TERCA après complément','photoRc3')}</div></div>}
    ...
    {step===9 && <div><h2 className="text-3xl font-black">📄 Rapport + mail</h2><SolNotice/><textarea value={data.emails} onChange={e=>update('emails', e.target.value)} className="h-28 w-full rounded-2xl border p-4" placeholder="responsable@secab.fr; client@mail.com"/><div className="mt-4 flex flex-wrap gap-3"><button onClick={()=>setError('Export Word : module à connecter au générateur de rapport final.')} className="rounded-2xl bg-blue-600 px-6 py-4 font-black text-white">📄 Générer rapport Word</button><button onClick={()=>setError('Transmission mail : préparer le rapport puis envoyer via l’application mail du téléphone ou du poste.')} className="rounded-2xl bg-green-600 px-6 py-4 font-black text-white">📧 Transmettre par mail</button></div></div>}
